"""
Comprehensive js/components.js update with Credits Page, Source Inspector,
and Source ID badges across all cards and views.
"""
...
import { SOURCES_REGISTRY, AUDIT_METADATA, getSourceById } from './sources.js';
...
/**
 * ==========================================================================
 * DEDICATED CREDITS & SOURCES PAGE (/credits)
 * ==========================================================================
 */
export function renderCreditsView(categoryFilter = 'ALL', searchQuery = '') {
  ...
  if (categoryFilter !== 'ALL') {
    list = list.filter(s => {
      ...
      if (filter === 'DATA') return type === 'DATA';
      if (filter === 'NEWS') return type === 'NEWS';
      if (filter === 'LOGOS') return type === 'LOGO';
      if (filter === 'FONTS') return type === 'FONT';
      if (filter === 'LIBRARIES') return type === 'LIBRARY';
      if (filter === 'APIS') return type === 'API' || type === 'DATA';
      if (filter === 'AI') return type === 'AI';
      return cat.includes(filter) || type === filter;
    });
  }
    ...
    <div class="bl-credits-view">
      ...
      <section class="compliance-header bl-hud-panel">
        ...
        <div class="compliance-header-content">
          <span class="compliance-kicker">BLUEGUN MASTER PROVENANCE REGISTER // AUDIT ID: 2026-BG-SRC</span>
          <h1 class="compliance-title">BLUEGUN — COPYRIGHT, CREDITS & SOURCES</h1>
          <p class="compliance-subtitle">
            "Transparency for every asset, dataset, library and external resource used by BLUEGUN."
          </p>
          <div class="credits-meta-row">
            <span class="audit-chip">🟢 Status: <strong>VERIFIED COMPLIANT</strong></span>
            <span class="audit-chip">📁 Registered Sources: <strong>${SOURCES_REGISTRY.length} Items</strong></span>
            <span class="audit-chip">📅 Last Audit: <strong>August 25, 2026</strong></span>
          </div>
        </div>
      </section>

      <!-- 2. BLUEGUN ORIGINAL CONTENT OWNERSHIP NOTICE -->
      <div class="compliance-section bl-card bl-hud-panel original-content-panel">
        <div class="panel-header-clean">
          <h2 class="panel-section-title"><span class="title-bracket">[</span> BLUEGUN ORIGINAL CONTENT <span class="title-bracket">]</span></h2>
          <span class="panel-tag cyan">PROPRIETARY IP</span>
        </div>

        <div class="legal-notice-box">
          <h3 class="legal-box-title">
            © 2026 BLUEGUN. All rights reserved for original project content, subject to the rights of third-party materials identified on this page.
          </h3>
          <p>
            The following software, branding, algorithmic systems, and design elements are original intellectual creations of BLUEGUN:
          </p>
          <div class="original-assets-grid">
            <div class="asset-bullet"><strong>BLUEGUN Name & Branding:</strong> Original trademark & identity</div>
            <div class="asset-bullet"><strong>Original Vector Logo:</strong> Custom SVG cyber geometry</div>
            <div class="asset-bullet"><strong>UI/UX Design Architecture:</strong> Layouts, cyber glow panels & HUD</div>
            <div class="asset-bullet"><strong>Original CSS & Design Tokens:</strong> Vanilla CSS custom design system</div>
            <div class="asset-bullet"><strong>Original JavaScript Modules:</strong> ES6 architecture & router</div>
            <div class="asset-bullet"><strong>Original Backend Code:</strong> FastAPI application & schemas</div>
            <div class="asset-bullet"><strong>Database Architecture:</strong> SQLite/PostgreSQL schema designs</div>
            <div class="asset-bullet"><strong>Original Animations:</strong> Keyframe pulse & scanline visualizers</div>
            <div class="asset-bullet"><strong>Original Vector Icons:</strong> Bespoke inline SVG cyber icons</div>
            <div class="asset-bullet"><strong>Scouting Editorial:</strong> Written descriptions & tactical analyses</div>
            ...
            <div class="asset-bullet highlight"><strong>TRANSFER PROBABILITY:</strong> Multi-parameter predictive model</div>
            <div class="asset-bullet"><strong>Vector Chart Engine:</strong> Zero-dependency pure math SVG visualizer</div>
          </div>
        </div>
      </div>
      ...
      <div class="credits-filter-bar bl-hud-panel">
        <div class="search-input-wrap">
          <input type="text" id="creditsSearchInput" class="bl-input" placeholder="Search sources by ID, name, creator, license, or component..." value="${searchQuery}" />
        </div>

        <div class="filter-chips-group">
          ${['ALL', 'IMAGES', 'DATA', 'NEWS', 'LOGOS', 'FONTS', 'LIBRARIES', 'APIS', 'AI', 'OTHER'].map(cat => `
      ...
      <!-- Master Sources Stream -->
      <div class="credits-sources-stream">
        ${list.length > 0 ? list.map(s => `
          <article class="source-registry-card bl-hud-panel" id="src-${s.id}">
            ...
            <div class="source-specs-grid">
              <div class="source-cell">
                <span class="lbl">CREATOR / PHOTOGRAPHER</span>
                <strong class="val">${s.creator || 'N/A'}</strong>
              </div>
              <div class="source-cell">
                <span class="lbl">COPYRIGHT / TRADEMARK HOLDER</span>
                <strong class="val">${s.copyright_holder || 'N/A'}</strong>
              </div>
              <div class="source-cell">
                <span class="lbl">LICENSE TYPE</span>
                <strong class="val cyan">${s.license || 'SOURCE / LICENSE VERIFICATION REQUIRED'}</strong>
              </div>
              <div class="source-cell">
                <span class="lbl">DATE ACCESSED</span>
                <strong class="val">${s.date_accessed || '2026-08-25'}</strong>
              </div>
            </div>

            ${s.attribution_text ? `
              <div class="source-attr-box">
                <span class="lbl">MANDATORY ATTRIBUTION:</span>
            ...
            <div class="source-card-footer">
              ...
              <span class="source-audit-tag">Audited & Documented in data/sources.json</span>
            </div>
          </article>
        `).join('') : '<p class="empty-state">No registered sources match the selected criteria.</p>'}
      </div>

      <!-- 19. BLUE LOCK INSPIRATION & TRADEMARK NOTICE -->
      <section class="compliance-section bl-card bl-hud-panel" style="margin-top: 32px;">
        <div class="panel-header-clean">
          <h2 class="panel-section-title"><span class="title-bracket">[</span> INSPIRATION & TRADEMARK NOTICE <span class="title-bracket">]</span></h2>
          <span class="panel-tag gold">INDEPENDENT PROJECT</span>
        </div>

        <div class="legal-notice-box">
          <div class="quote-notice">
            "BLUEGUN is an independent football transfer and scouting project. BLUEGUN is not affiliated with, endorsed by, sponsored by, or officially connected to Blue Lock or its creators/rightsholders."
          </div>
          <p>
            BLUEGUN does not use official Blue Lock character artwork, manga panels, anime screenshots, soundtracks, or promotional materials. All software, UI design, color schemes, and vector elements are original creations inspired by competitive athletic ego dynamics.
          </p>
        </div>
      </section>

      <!-- 29. FINAL COPYRIGHT NOTICE -->
      <section class="final-copyright-section bl-hud-panel">
        <div class="final-copyright-box">
          ...
          <p class="final-cr-text">
            BLUEGUN original software, interface design, original graphics, original written material, and original analytical systems are protected to the extent permitted by applicable law. Third-party content, trademarks, photographs, data, software and other materials remain the property of their respective owners and are used according to their applicable licences, permissions, terms, or other lawful basis.
          </p>
          <div class="final-audit-badge">
            <span>Last Credits Audit: <strong>August 25, 2026</strong></span>
          </div>
        </div>
      </section>
      ...
      <div class="footer-container">
        ...
        <div class="footer-col brand-col">
          ...
          <div class="footer-copyright-main">
            <strong>© 2026 BLUEGUN. All rights reserved for original project content, subject to the rights of third-party materials identified on the Credits page.</strong>
          </div>
        </div>
...
print("Successfully updated js/components.js with dedicated Credits & Sources page, micro source buttons, and inspector modal!")
